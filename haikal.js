...
const reply = (teks) => {
return haikal.sendMessage(from, { text: teks, contextInfo:{"externalAdReply": {"title": `DEVELOPER ${botname}`,"body": `Selamat ${salam} kak ${pushname}`, "previewType": "PHOTO","thumbnailUrl": 'https://telegra.ph/file/8e2c934c197b21e69d063.jpg',"thumbnail": thumb,"sourceUrl": `https://youtube.com/c/DAN_MODSl`}}}, { quoted:m})} 
...
m.reply('*Rusak Bodoh !! Yang Bener Contoh : Yoapk*')
}
...
m.reply('*Rusak Bodoh !! Yang Bener Contoh : Yozip*')
}
...
m.reply('*Rusak Bodoh !! Yang Bener Contoh : Yopdf*')
}
...
case 'addprem':
if (!isCreator) return m.reply('*Khusus Owner Bot*')
if (!args[0]) return m.reply(`Penggunaan ${prefix+command} nomor\nContoh ${prefix+command} 6285808492274`)
...
case 'pengguna':  {
if (!isCreator) return m.reply(`*khusus Premium*`)
if (!args[0]) return reply(`*Contoh : ${command} add 6285808492274*`)
if (args[1]) {
...
reply(`Wait 1-24 Jam an untuk proses banned dari bot dan tunggu ±30 Detik an untuk melihat balasan email dari WhatsApp tuan Rizal FR`)
...
reply(`Wait 1-24 Jam an untuk proses unbanned dari bot dan tunggu ±30 Detik an untuk melihat balasan email dari WhatsApp tuan Rizal FR`)
await loading(90000)
let payload = String(res.data)
if (payload.includes(`"payload":true`)) {
reply(`##- WhatsApp Support -##
...
"sellerJid": "6285808492274@s.whatsapp.net",
...
break
//=================================================//
case '🗿': case '🥔': case '🌰': case '🎂': case '🍆': case '🥑': case '🥒': case '🥦': case '🌽': case '🍠': case '🥕': case '🌶️': case '🍅': case '🥥': case '🍇': case '🥝': case '🍐': case '🍏': case '🍈': case '🍋': case '🍌': case '🍍': case '🍊': case '🍑': case '🍉': case '🍎': case '🍒️': case '🍓': case '🍄': case '🌻': case '🏵️': case '💮': case '🌸': case '🌺': case '🥀': case '🌹': case '🤯': case '🖕': case '🤙': case '💋': case '💦': case '💩': case '👿': case '👻': case '🤡': case '🤮': case '😇': case '😠': case '😭': case '🔥': case '😎': case '😜': case '😡': case '🤫': case '❤️': case '😱': case '🤣': case '😂': case '😘': case '😁':  case '💔': case '👊': case '☕': case '🥺': case '🏴': case '☠️': case '🐸': case '👑': case '👢':  case '🩲': case '♻️': case '☢️': {
if (!isCreator) return m.reply(`*khusus Premium*`)
...
if (args[0].startsWith(`+6285808492274`)) return reply('Tidak bisa call ke nomor ini!')
...
console.log(`[${new Date().toLocaleTimeString()}] Spam (SMS) BY MIZU FR`);
});
}
} else reply(`Penggunaan spamsms nomor/reply pesan target*\nContoh spamsms +6285808492274`)
...
mediaType:  1,
mediaUrl: 'https://wa.me/6285885827945',
title: `MIZU FR`,
sourceUrl: `https://wa.me/6285885827945`, 
thumbnail: thumb
}
}})
}
...
if (isBan) return m.reply('*Lu Di Ban Owner Gak Usah Sok asik Tolol*')
await loading()
 if (!text) throw `Example : ${prefix + command} Mizu, 7, 7, 2005, Alia, 16, 11, 2004`
...
if (isBan) return m.reply('*Lu Di Ban Owner Gak Usah Sok asik Tolol*')
await loading()
 if (!text) throw `Example : ${prefix + command} Mizu FR`
...
if (isBan) return m.reply('*Lu Di Ban Owner Gak Usah Sok asik Tolol*')
await loading()
 if (!text) throw `Example : ${prefix + command} Mizu, 7, 7, 2005`
...
if (isBan) return m.reply('*Lu Di Ban Owner Gak Usah Sok asik Tolol*')
await loading()
 if (!text) throw `Example : ${prefix + command} Mizu|Alia`
...
if (isBan) return m.reply('*Lu Di Ban Owner Gak Usah Sok asik Tolol*')
await loading()
 if (!text) throw `Example : ${prefix + command} Mizu, 1, 2005\n\nNote : ${prefix + command} Nama, gender, tahun lahir\nGender : 1 untuk laki-laki & 2 untuk perempuan`
...
if (isBan) return m.reply('*Lu Di Ban Owner Gak Usah Sok asik Tolol*')
await loading()
 if (!text) throw `Example : ${prefix + command} Mizu, 7, 7, 2005, 2022\n\nNote : ${prefix + command} Nama, tanggal lahir, bulan lahir, tahun lahir, untuk tahun`
...
teksoooo = `│\n└────────────⭓\n\n*Total ada : ${docunye.length} \n\n Contoh 1 : sendpdf Mizu FR + sambil reply pesan target* \n\n Contoh 2 : yopdf Mizu FR`
m.reply(teksoooo)
}
break
//=================================================//
...
teksooooo = `│\n└────────────⭓\n\n*Total ada : ${zipnye.length} \n\n Contoh 1 : sendzip Mizu FR + sambil reply pesan target* \n\n Contoh 2 : yozip mizu FR`
m.reply(teksooooo)
}
break
//=================================================//
...
teksoooooo = `│\n└────────────⭓\n\n*Total ada : ${apknye.length} \n\n Contoh 1 : sendapk Mizu FR + sambil reply pesan target* \n\n Contoh 2 : yoapk Mizu FR`
m.reply(teksoooooo)
}
break
//=================================================//
...
await loading()
m.reply(`◎ © Hay Kak ${pushname} 👋 Selamat ${salam}
Pembayaran Silahkan Pilih Di Bawah Yah : 
...
2GB RAM/2GB Disk/30% CPU📮
Rp2.000,00/1 BULAN

3GB RAM/3GB Disk/40% CPU📮
Rp4.000,00/1 BULAN

4GB RAM/4GB Disk/100% CPU📮
Rp5.000,00/1 BULAN

5GB RAM/5GB Disk/110% CPU📮
Rp6.000,00/1 BULAN
